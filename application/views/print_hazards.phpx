...
if (isset($img)){
	...
	$query = "";
	if (count($hzs))
		$query .= "SELECT CONCAT(hazard_id,'h') id, hazard_id, center_latitude, center_longitude, title, information, location, hazard_image, hazard_icon, '' site_photo, status, COALESCE(start_date,null) start_date, COALESCE(end_date,null) end_date, hazard_control FROM hazard_tb WHERE active=1 and hazard_id IN (".implode(',', $hzs).")";
	if (count($hzs) && count($sts))
		$query .= " UNION ";
	if (count($sts))
		$query .= "SELECT CONCAT(site_id,'s') id, site_id, center_latitude, center_longitude, site_name title, null information, site_location location, site_img hazard_image, hazard_icon, site_photo, null status, null start_date, null end_date, null hazard_control FROM site_tb WHERE active=1 and site_id IN (".implode(',', $sts).")";
	//dump_exit($query);
	...
	foreach($result as $row){
		$hazards[] = array(
			'site_hazard' =>$row->id,
			'hazard_id' => isset($row->hazard_id) ? $row->hazard_id : $row->site_id,
...
<?php 	
	foreach ($_POST['directions'] as $dir)
	{
		echo $dir.'<br>';
	}
?>
