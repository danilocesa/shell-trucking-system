...
// **************** Functions ***********************
	...
	function addRoute() {
		...
		$("#opts_sub").click(function(){
			$("#point_notify").text("Starting points");
			map.controls[google.maps.ControlPosition.TOP_CENTER].push(document.getElementById('point_notify'));
			...
			//Manual Directions
				...
				map.controls[google.maps.ControlPosition.TOP_CENTER].push(document.getElementById('start_point'));
				google.maps.event.addListener(map,"click", function(location) {
					clickCounter += 1;
					if(clickCounter == 1){
						$("#start_point").hide();
			...
			for(var j = 0; j < haz_stat.length; j++){
				if(boxpolys[i].getBounds().contains(new google.maps.LatLng(haz_stat[j].getPosition().lat(), haz_stat[j].getPosition().lng()))){
					var haz_latlng = new google.maps.LatLng(haz_stat[j].getPosition().lat(), haz_stat[j].getPosition().lng());
							...
							count_li = $("ol#marker-list li").size()+1;
