...
if(!function_exists('audit_insert')){
    function audit_insert($desc,$ids) {
        $CI =& get_instance();
        $login_info = get_session("login");
        $user_info = $CI->dan_model->select_where("user_tb",array("user_id"=>$login_info['user_id']));
        $CI->dan_model->inserting("audit_trail_tb",array("user_id"=>$user_info->user_id,"description"=>$desc,"firstname"=>$user_info->firstname,"date"=>date("Y-m-d H:i:s"),"email"=>$user_info->email,"ids"=>$ids));    
        return TRUE;
    }
}
