...
if (isset($img)){
	...
	if (count($hzs) && count($sts))
		...
		$query .= " UNION ";
	if (count($sts))
		$query .= "SELECT CONCAT(site_id,'s') id, site_id, center_latitude, center_longitude,site_name, null, site_location, site_img, hazard_icon, site_photo, null, null, null, null FROM site_tb WHERE active=1 and site_id IN (".implode(',', $sts).")";
	//dump_exit($query);
	...
	foreach($result as $row){
		$hazards[] = array(
			...
			'center_latitude' => $row->latitude,
	 		'center_longitude' => $row->longtitude,
	 		'title' => $row->title,
